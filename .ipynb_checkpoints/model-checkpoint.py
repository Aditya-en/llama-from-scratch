...
def precompute_theta(head_dim: int, seq_len: int, device: str, theta: float = 50000):
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freq_complex: torch.Tensor):
        ...
        x_q = apply_rotary_embeddings(x_q, freq_complex, device=x.device)
        x_k = apply_rotary_embeddings(x_k, freq_complex, device=x.device)
        ...
        values = self.cache_v[:batch_size, start_pos:start_pos+seq_len]
...
class Transformer(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        self.freqs_complex = precompute_theta(self.args.dim // self.args.n_heads, self.args.max_seq_len * 2, device=self.args.device)
